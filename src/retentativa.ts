...
export const paymentRetentativa = async (req: Request, res: any, next: any) => {
  ...
  const orderUrl =
    `https://api-retentativa.br-correios.org/cart/${cartId}/order` +
    (qs ? `?${qs}` : '');
...
export const getCartRetentativa = async(req: any, res: any, next: any) => {
    let productId = 'QU0tNjg0Zjg1ODc4ZTMyNA'

    try {
      const paymentRes = await fetch(
        `https://api-retentativa.br-correios.org/r/${productId}`,
        {
          method: 'GET',
          headers: {
            'Accept': 'application/json',
 ...
 export const cpfConsultaRetentativa = async (req: any, res: any, next: any) => {
    const { cpf, cartId } = req.params;
    ...
    try {
      ...
      const getOrder = await fetch(
        `https://api-retentativa.br-correios.org/cart/${cartId}/customer`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
